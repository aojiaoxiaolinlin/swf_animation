...
impl MovieClip {
    ...
    fn child_by_depth(&mut self, depth: Depth) -> Option<Arc<RefCell<DisplayObject>>> {
    ...
    pub fn set_flags(&mut self, flags: MovieClipFlags) {
        self.flags.insert(flags);
    }
    pub fn parse_swf(&mut self, library: &mut MovieLibrary) {
        let swf = self.swf.clone();
        let mut reader: Reader<'_> = Reader::new(&swf.data()[..], swf.version());
        ...
        match place_object.action {
            ...
            PlaceObjectAction::Replace(id) => {
                ...
                if let Some(child) = self.child_by_depth(place_object.depth.into()) {
                    child.borrow_mut().replace_with(id, library);
                    child
                        .borrow_mut()
                        .apply_place_object(&place_object, swf.version());
                    child.borrow_mut().set_place_frame(current_frame);
                }
            }
    ...
    fn instantiate_child(
        ...
        match child {
            Ok(child) => {
                child.borrow_mut().set_depth(depth);
                child.borrow_mut().set_place_frame(self.current_frame);
                child
                    .borrow_mut()
                    .apply_place_object(&place_object, self.swf.version());
                if let Some(name) = &place_object.name {
                    let name = name
                        .to_str_lossy(SwfStr::encoding_for_version(self.swf.version()))
                        .into_owned();
                    child.borrow_mut().set_name(Some(name));
                }
                if let Some(clip_depth) = place_object.clip_depth {
                    child.borrow_mut().set_clip_depth(clip_depth);
                }
                child.borrow_mut().post_instantiation(library);
                child.borrow_mut().enter_frame(library);
                Some(child)
            }
            Err(_e) => None,
        }
    ...
    fn instantiate_by_id(
        ...
        id: CharacterId,
        library: &mut MovieLibrary,
    ) -> anyhow::Result<Arc<RefCell<DisplayObject>>> {
    ...
    pub fn query_movie_clip(&mut self, arg: &str) -> Option<Arc<RefCell<Self>>> {
        if self.name() == Some(arg.to_owned()) {
            return Some(Arc::new(RefCell::new(self.clone())));
        } else {
            for (_, child) in self.raw_container_mut().display_objects_mut() {
                let child = Arc::make_mut(child);
                let child = RefCell::get_mut(child);
                match child {
                    DisplayObject::MovieClip(movie_clip) => {
                        if movie_clip.as_ref().borrow().name() == Some(arg.to_owned()) {
                            let movie_clip = RefCell::get_mut(Arc::make_mut(movie_clip));
                            return Some(Arc::new(RefCell::new(movie_clip.clone())));
                        } else {
                            return movie_clip.borrow_mut().query_movie_clip(arg);
                        }
                    }
                    _ => {}
                }
    ...
    pub fn run_goto(&mut self, library: &mut MovieLibrary, frame: FrameNumber, is_implicit: bool) {
        ...
        if is_rewind {
            let children: SmallVec<[_; 16]> = render_list
                .iter()
                .filter(|display_object| display_object.as_ref().borrow().place_frame() > frame)
                .map(|display_object| display_object.as_ref().borrow().depth())
                .collect();
        ...
        let run_goto_command = |clip: &mut MovieClip,
            ...
            match (params.place_object.action, child_entry, is_rewind) {
                (_, Some(prev_child), true) | (PlaceObjectAction::Modify, Some(prev_child), _) => {
                    prev_child
                        .borrow_mut()
                        .apply_place_object(&params.place_object, movie.version());
                }
                (PlaceObjectAction::Replace(id), Some(prev_child), _) => {
                    prev_child.borrow_mut().replace_with(id, library);
                    prev_child
                        .borrow_mut()
                        .apply_place_object(&params.place_object, movie.version());
                    prev_child.borrow_mut().set_place_frame(params.frame);
                }
                ...
                | (swf::PlaceObjectAction::Replace(id), _, _) => {
                        ...
                        clip.instantiate_child(id, params.depth(), &params.place_object, library)
                    {
                        ...
                        child.borrow_mut().set_place_frame(params.frame);
    ...
    fn goto_remove_object(
        ...
        if !is_rewind {
            ...
            if let Some(child) = child {
                let depth = child.as_ref().borrow().depth();
...
impl From<MovieClip> for Arc<RefCell<DisplayObject>> {
    fn from(value: MovieClip) -> Self {
        Arc::new(RefCell::new(DisplayObject::MovieClip(Arc::new(
            RefCell::new(value),
        ))))
    }
}
...
impl TDisplayObject for MovieClip {
    ...
    fn post_instantiation(&mut self, library: &mut MovieLibrary) {
        if self.flags.contains(MovieClipFlags::POST_INSTANTIATED) {
            return;
        }
        self.flags.insert(MovieClipFlags::POST_INSTANTIATED);
